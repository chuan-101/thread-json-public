...
test('model extraction and normalization pull from multiple metadata locations and auto-group variants', () => {
  ...
  const messageWithAuthor = { ts: Date.UTC(2025, 0, 3), role: 'assistant', author_metadata: { model: 'claude-3.5-sonnet' }, text: 'b' };
  ...
  assert.equal(extractRawModel(messageWithAuthor), 'claude-3.5-sonnet', 'checks author_metadata');
  ...
  assert.equal(normModelFamily('claude-3.5-sonnet'), 'Claude 3.5', 'anthropic models normalize with family version');
  assert.equal(normModelFamily('gemini-1.5-flash'), 'Gemini 1.5', 'gemini variants keep numeric prefix');
  assert.equal(normModelFamily('command-r7b'), 'Command', 'vendor-prefixed models map by leading token');
